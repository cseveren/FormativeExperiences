...
/*	
twoway line gas_price year, lcolor(gs12) || ///
	   rbar p25_gas_price gas_price year, fcolor(gs12) lcolor(black) barw(.7) || ///
       rbar gas_price p75_gas_price year, fcolor(gs12) lcolor(black) barw(.7) || ///
       rspike p25_gas_price p0_gas_price year, lcolor(black) || ///
       rspike p75_gas_price p100_gas_price year, lcolor(black) || ///
       rcap p0_gas_price p0_gas_price year, msize(*.7) lcolor(black) || ///
       rcap p100_gas_price p100_gas_price year, msize(*.7) pstyle(p1) legend(off) ///
	   yti("Nominal Gasoline Price (USD)") xlabel(,nogrid) ylabel(,nogrid) xti("") ///
	   yline(0, lcolor(gs8) lstyle(grid)) saving(level, replace)
*/
...
graph export "./results/gasprice_variation_picture.png", replace
...
twoway line d2gp_bp year, lcolor(gs12) || ///
	   ...
	   ylab(-0.5 "-50%" 0 "0%" 0.5 "50%" 1 "100%")
graph export "./results/gasprice_variation_picture_d2.png", replace	

/*
collapse (median) gas_price d1gp_bp d2gp_bp ///
			(min) p0_gas_price=gas_price p0_d1gp_bp=d1gp_bp p0_d2gp_bp=d2gp_bp ///
			(p25) p25_gas_price=gas_price p25_d1gp_bp=d1gp_bp p25_d2gp_bp=d2gp_bp ///
			(p75) p75_gas_price=gas_price p75_d1gp_bp=d1gp_bp p75_d2gp_bp=d2gp_bp ///
			(max) p100_gas_price=gas_price p100_d1gp_bp=d1gp_bp p100_d2gp_bp=d2gp_bp, by(year)
...
twoway line d1gp_bp year, lcolor(gs12) || ///
       ...
       rcap p0_d1gp_bp p0_d1gp_bp year, msize(*.7) lcolor(black) || ///
       rcap p100_d1gp_bp p100_d1gp_bp year, msize(*.7) pstyle(p1) legend(off) ///
	   yti("Gas Price (year+1) / Gas Price (year)") xlabel(,nogrid) ylabel(,nogrid) xti("") ///
	   yline(0, lcolor(gs8) lstyle(grid)) saving(d1, replace)
...
twoway line d2gp_bp year, lcolor(gs12) || ///
       ...
       rcap p0_d2gp_bp p0_d2gp_bp year, msize(*.7) lcolor(black) || ///
       rcap p100_d2gp_bp p100_d2gp_bp year, msize(*.7) pstyle(p1) legend(off) ///
	   yti("Gas Price (year + 2) / Gas Price (year)") xlabel(,nogrid) ylabel(,nogrid) xti("Year") ///
	   yline(0, lcolor(gs8) lstyle(grid)) saving(d2, replace)   
	   
gr combine level.gph d1.gph d2.gph, xcommon col(1) ysize(10) xsize(6.8)
graph export "./results/gasprice_variation_picture.png", replace
graph export "./results/gasprice_variation_picture.pdf", replace
graph close
